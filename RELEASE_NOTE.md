...
- **Public overview card** — the QManager root page now shows a glanceable modem status card before login, with a dedicated Login button. Carrier, network type, signal quality, bands, model, and uptime are visible without authenticating; sensitive fields (IMEI, ICCID, IP, traffic) remain login-gated.
...
- **Tailscale routing simplified** — On RM551E firmware with `sdxpinn-patch`, Tailscale's magic IP (`100.x.x.x`) routing is handled at the firmware level, so QManager no longer adds workaround firewall zones or mwan3 ipset entries for Tailscale. NetBird's path is unchanged. Existing devices automatically clean up the legacy Tailscale firewall zone on the next install or software update.
